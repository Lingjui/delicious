...
$(".input_text_item2 input").live("focus",function(){
	$(this).parent().append("<div class='tips_yl'><span><em class='num'></em>克</span><span><em class='num'></em>毫升</span></div>");
	$(this).parents(".row1_box_half").css("z-index","100");
...
$(".input_text_item2 input").live("blur",function(){
	...
	setTimeout(function(){
		...
		_this.parents(".row1_box_half").css("z-index","1");
	},300);
	
	
});

$(".input_text_item2 input").live("keyup",function(e){
...
function aca_tips(){
	Alert("提示", "id:rulebox", "530", "300", "false", "", "true", "img");
}
var can_submit=true;
function saveReport() {
	...
	$("#uploadform").ajaxSubmit(function(message) {
		can_submit=true;
		setTimeout(function(){do_submit(3);},60000);
		if(message != ''){
...
function addStep(ele){
	var p = ele.parents(".stepitem");
	var guid = guidGenerator();
	p.after("<div class='stepitem'><div class='stepnum'></div><div class='stepupload'><iframe src='/iframe/pic_step.php?step="+guid+"' frameborder='0' height='180' width='180' scrolling='no'></iframe><input type='hidden' id='step_img_"+guid+"' name='step_img[]' /></div><div class='stepright'><div class='stepst'><div class='step_scselect'><div class='step_scselect_item'><span class='t'>此步骤所用食材：</span><input type='hidden' value='' class='step_sc_input' name='step_sc_input[]'><div class='c'><dl class='zl clearfix'><dt class='zl'><span class='s1'>主料</span><span class='s2'>步骤用量（可修改）</span></dt></dl><dl class='fl clearfix'><dt class='fl'><span class='s1'>辅料</span><span class='s2'>步骤用量（可修改）</span></dt></dl></div></div><p>此步骤所用食材：</p><div class='selectitem_l'><p>请选择食材</p></div></div><div class='step_timeselect'><p>此步骤所用时间：</p><div class='timeinput'><input type='text' name='step_time[]'></div><span class='timedw_select'>分钟</span></div></div><textarea placeholder='请填写步骤描述' name='step_text[]'></textarea></div><div class='step_rightbtns'><a href='####' class='moveUp'></a><a href='####' class='moveDown'></a><a href='####' class='addStep'></a><a href='####' class='delStep'></a></div></div>");
	relist_step();
	bindSteprightEvents();
	bindStepScSelect();
}
...
function bindSteprightEvents(){
	...
	$(".moveUp").click(function(){moveUp($(this));});
	$(".moveDown").click(function(){moveDown($(this));});
	$(".addStep").click(function(){addStep($(this));});
	$(".delStep").click(function(){delStep($(this));});
}
...
function reshowSclistInStep(ele){
	...
	c += "<dd class='dd2'><a href='javascript:void(0);'' class='addinstep'>+ 增加一行</a></dd>"
	...
	d += "<dd class='dd2'><a href='javascript:void(0);'' class='addinstep'>+ 增加一行</a></dd>"
...
function addScInStep(ele){
	...
	$.each(add_zl,function(){
		...
		var txt = "<div class='row1_box_half'><div class='input_text_item1'><input name='zlsc"+String((l+1))+"' type='text' placeholder='请填写食材名称' value="+$(this).find("input").first().val()+"></div><div class='input_text_item2'><input name='zlyl"+String((l+1))+"' type='text' placeholder='请填写用量' value="+$(this).find("input").last().val()+"></div><span class='delsc'></span></div>"
		$("#zlinputs").append(txt);
	});
	...
	var t = "";
	if(tt == ""){t = "\u8bf7\u9009\u62e9\u98df\u6750"}else if(tt.length <= 9){t = tt}else if(tt.length >9){t = tt.substring(0,9)+"..."}
	e.find(".selectitem_l p").html(t);
}
var active = -1;
var lastKeyValue = '';
var lastKeyPressCode = 0;
//var default_value = $("#iptstr1").attr('defaultval');
var default_value = "";
...
/*function cut_img(img){
	Alert("拖动选框修改成品图的展示范围", "iframe:/iframe/upload_cut_pic.php?pid=" + img, "700", "500", "false", "", "true", "img");
}
...
/*function do_submit(type){
	if(type == 2){
		$('.submit_2').val('保存中...请稍候');
		$('.submit_2').css('color','yellow');
	}else if(type == 1){
		$('.submit_1').val('提交中...请稍候');
	...
	function IE_Flash(){   
		try{   
			...
			{
				$(".muluploadbox .tip").html("你的浏览器安装falsh版本过低,可能会影响批量上传功能的使用 <a href='http://get.adobe.com/cn/flashplayer/' target='_blank'>点击下载新版flash ></a>");
				$(".muluploadbox_done .tip").html("你的浏览器安装falsh版本过低,可能会影响批量上传功能的使用 <a href='http://get.adobe.com/cn/flashplayer/' target='_blank'>点击下载新版flash ></a>");
			...
			$(".muluploadbox .tip").html("你的浏览器未安装falsh，安装flash后可使用批量上传功能哦~ <a href='http://get.adobe.com/cn/flashplayer/' target='_blank'>点击下载 ></a>");
			...
			$(".muluploadbox_done .tip").html("你的浏览器未安装falsh，安装flash后可使用批量上传功能哦~ <a href='http://get.adobe.com/cn/flashplayer/' target='_blank'>点击下载 ></a>");
